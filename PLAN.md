...
- local machine에서 Codex app-server 또는 Claude Code CLI 실행
...
- Codex app-server 또는 Claude Code CLI를 사용할 수 있다.
...
7. Agent Runner
선택된 provider의 subprocess 실행, startup 또는 stream 처리, continuation turn, timeout과 stall 감지를 담당한다.
...
- `internal/agent`는 provider-neutral runner contract와 transcript helper를 담당한다.
- `internal/agent/codex`는 Codex app-server adapter다.
- `internal/agent/claude`는 Claude Code CLI adapter다.
...
- `SessionID`는 provider가 turn ID를 노출하면 `<conversation_id>-<turn_id>`, 아니면 provider-specific synthetic ID를 사용한다.
...
13. issue branch는 harness-managed handoff branch로 간주하며, retry 중 remote issue branch history를 현재 workspace `HEAD`로 대체할 수 있다.
14. workspace-local runtime artifacts such as review files, tool caches, and scratch output must stay under `.harness/` so PR handoff can ignore them safely.
15. PR handoff가 성공하면 issue를 `Done`으로 전환한다.
16. PR handoff가 실패하면 attempt failure로 기록하고 retry policy를 적용한다.
17. issue가 terminal이면 session을 중단하고 workspace를 정리한다.
18. issue가 non-active non-terminal이면 session만 중단하고 cleanup은 정책에 따라 분리한다.
...
7. 첫 번째 agent의 verdict가 `done`이고 검증을 통과하면, stale artifacts를 지운 뒤 두 번째 review agent를 실행한다 (two-pass consensus).
...
- `permission_mode`
  ...
  - Claude Code CLI가 지원하는 값을 pass-through로 취급한다
- `allowed_tools`
...
- `claude.command`도 shell command string 그대로 유지하고 실제 expansion은 `bash -lc`에 맡긴다.
...
- `agent.provider`가 지원되는 값인지 여부
- 선택된 provider의 command 존재 여부
...
- polling cadence, concurrency, active 또는 terminal states, workspace root, hooks, prompt, provider launch config는 future dispatch에 즉시 적용한다.
...
- stdout: provider-specific stream
...
Codex provider는 다음 메시지를 순서대로 보낸다.
...
- Codex는 `thread_id`를 `thread/start` 결과에서 읽는다.
- `turn_id`는 각 `turn/start` 결과에서 읽는다.
- Codex `SessionID`는 `<thread_id>-<turn_id>`로 만든다.
- Claude는 CLI `session_id`를 `conversation_id`로 사용하고 turn별 `SessionID`는 synthetic value로 만든다.
...
- Codex는 success 후 issue가 active면 같은 live process와 같은 `thread_id`로 다음 `turn/start`를 호출한다.
- Claude는 success 후 issue가 active면 새 CLI process를 시작하고 `--resume <conversation_id>`로 이어서 실행한다.
- continuation turn에는 전체 task prompt를 재전송하지 않는다.
- Codex app-server subprocess는 run이 끝날 때까지 유지한다.
