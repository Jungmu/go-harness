...
이 문서는 `Symphony`의 상위 개념과 이 저장소의 [SPEC.md](../SPEC.md)를 기준으로, 개인 또는 소규모 팀이 운영할 수 있는 Go 기반 하네스 시스템의 v1 설계를 정의한다.
...
1. 이 저장소의 [SPEC.md](../SPEC.md)
2. 이 문서 `PLAN.md`
3. `../elixir` reference implementation
...
4. Orchestrator
poll loop, concurrency 제어, `claimed`/`running`/`retry` 상태 관리, dispatch, stop, reconcile을 담당한다.
